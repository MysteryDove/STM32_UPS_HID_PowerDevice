...
`src/main.c` now runs a full UART bootstrap state machine for SPM2K:

- send/verify a heartbeat frame first (`'Y'` expecting `"SM\r\n"`)
- enqueue the full constant LUT
- enqueue one full dynamic LUT pass
...
Note: `src/main.c` includes substantial project-specific USER CODE in this repo.
