...
static void fault_fail_fast_reset(void)
{
  __disable_irq();
  NVIC_SystemReset();
  while (1)
...
void USB_HP_CAN1_TX_IRQHandler(void)
{
  ...
  // TinyUSB owns the USB peripheral/interrupt handling.
  // Calling HAL_PCD_IRQHandler() here can consume/clear USB events that TinyUSB
  // needs (especially around bus reset / re-enumeration, e.g. VM attach/detach).
  //
